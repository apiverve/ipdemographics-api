...
class IpdemographicsValidationException implements Exception {
  final List<String> errors;

  IpdemographicsValidationException(this.errors);

  @override
  String toString() => 'IpdemographicsValidationException: ${errors.join("; ")}';
}
...
/// IP Demographics API client.
///
/// For more information, visit: https://apiverve.com/marketplace/ipdemographics?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [ip] (required) - IPv4 or IPv6 address [format: ip]
class IpdemographicsClient {
  final String apiKey;
  ...
  IpdemographicsClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/ipdemographics',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [IpdemographicsValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw IpdemographicsValidationException(errors);
    }
  }

  /// Execute a request to the IP Demographics API.
  ///
  /// Parameters are validated before sending the request.
  Future<IpdemographicsResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw IpdemographicsException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return IpdemographicsResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw IpdemographicsException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw IpdemographicsException('Resource not found');
      } else {
        throw IpdemographicsException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is IpdemographicsException) rethrow;
      throw IpdemographicsException('Request failed: $e');
    }
  }
...
/// Exception thrown by the IP Demographics API client.
class IpdemographicsException implements Exception {
  final String message;

  IpdemographicsException(this.message);

  @override
  String toString() => 'IpdemographicsException: $message';
}
